...
const AuthPage = () => {
  ...
  const handleConfirmOtp = () => {
    ...
    alert('UI-only: code verified (no backend)');
  ...
  const handleSubmit = (e) => {
    ...
    const data = {
      ...
      otp: isSignUp ? (showOtp ? otp : undefined) : undefined,
    };

    console.log(isSignUp ? 'Sign Up' : 'Sign In', data);
    alert(`${isSignUp ? 'Sign Up' : 'Sign In'} submitted (check console)`);
  };
  ...
  return (
    <>
      <div className="auth-container">
        <div className="left">
          <h1>SkinIQ</h1>
          <p className="intro">Welcome to SkinIQ — Your AI-Powered Skin Health Monitoring and Personalized Care Platform</p>
        </div>

        <div className="right">
          <div className="auth-forms" role="region" aria-label="Authentication forms">
            <div className="tab-toggle" aria-hidden="false">
              <button
            ...
            <div className={`form-slider ${isSignUp ? 'signUp' : ''}`} aria-live="polite">
              <form ref={signInRef} className="panel" onSubmit={handleSubmit} autoComplete="on" aria-label="Sign in form">
                <h3>Welcome Back</h3>
                ...
                <div className="muted">Forgot password? <button type="button" onClick={() => alert('Reset flow')} style={{ border: 'none', background: 'transparent', color: '#2d8cff', cursor:'pointer' }}>Reset</button></div>
              </form>

              <form ref={signUpRef} className="panel" onSubmit={handleSubmit} autoComplete="on" aria-label="Sign up form">
                ...
                <div className="phone-verify" aria-label="Mobile number input" style={{ marginTop: 6 }}>
                  <div style={{ flex: 1 }}>
                    <PhoneInput
                      country={'us'}
                      value={mobile}
                ...
                <div className="muted">Already have an account? <button type="button" onClick={() => handleSwitch(false)} style={{ border: 'none', background: 'transparent', color: '#3a86ff', cursor:'pointer' }}>Sign In</button></div>
              </form>
            </div>
          </div>
        </div>
